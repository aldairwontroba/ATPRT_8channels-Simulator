 ...
 */
 ...
 //  verifica o time step do ATP
  ...
  startup("C:\\Users\\aldair\\Desktop\\SegundoPrograma\\AudioTransmiterATP.exe");
...
void c_teste_m__(double xdata_ar[], double xin_ar[], double xout_ar[], double xvar_ar[])
{
	...
	double channel8 = xin_ar[11];
  	...
  	double ranenable = xdata_ar[8];
	double ranres = xdata_ar[9];
	double ranvpp = xdata_ar[10];
	double ranvnn = xdata_ar[11];
	double minvpp = xdata_ar[12];
	double minvnn = xdata_ar[13];
	...
	if(timebufferATP>=(Audiotime+timestepaudio*BUF_SIZE)){
		...
		int CanIWrite = no; 
 		...
 		t1 = ComunicationOfInfo.tecla1;
 		t2 = ComunicationOfInfo.tecla2;
 		t3 = ComunicationOfInfo.tecla3;
 		t4 = ComunicationOfInfo.tecla4;
...
/// variaveis aleatorias diodo
  	if((ranenable==1)&&(t1>0)){
 		xvar_ar[4] = rand() % (int)ranres;
 		xvar_ar[5] = minvpp + (rand() % (int)ranvpp);
 		xvar_ar[6] = minvnn + (rand() % (int)ranvnn);
    	xout_ar[4] = xvar_ar[4];
    	xout_ar[5] = xvar_ar[5];
    	xout_ar[6] = xvar_ar[6];
  	}else{
